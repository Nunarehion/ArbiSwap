...
class JsonFileHandler(logging.FileHandler):
    def emit(self, record):
        if hasattr(record, 'single_file') and record.single_file:
            directory_name = f"logs/{record.single_file}"
            os.makedirs(os.path.dirname(directory_name), exist_ok=True)
            log_filename = f"{directory_name}.json"
            detail = {}
        else:
            directory_name = f"logs/{(record.filename)[:-3] if not(hasattr(record, 'file')) else record.file}"
...
if __name__ == "__main__":
    logger.debug({'event': 'user_login', 'user_id': 123,
                 'status': 'success'}, extra={'file': 'run/user_action'})
